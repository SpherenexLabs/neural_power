...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeLive = onValue(liveDataRef, (snapshot) => {
      ...
      if (data) {
        ...
        const mappedData = {
          ...
          pf: data.PF || 0,
          state: data.state || "NORMAL",
          dc_current: 0, // Will be updated from root level
          dc_voltage: 0  // Will be updated from root level
        };
    ...
    const dcDataRef = ref(database, '38_AC_Power_Facter');
    const unsubscribeDc = onValue(dcDataRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        console.log('Firebase ROOT data received:', data);
        console.log('DC_Current:', data.DC_Current);
        console.log('DC_Voltage:', data.DC_Voltage);
